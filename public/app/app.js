  ...
  if(vendorGrid){
    ...
    function loadVendors(){
      ...
      jsonFetch('/api/vendors/smart-search',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)}).then(function(payload){
        ...
        vendorGrid.innerHTML=payload.data.map(function(v){var reasons=(v.match_reasons||[]).map(function(r){return '<span>'+escHtml(r)+'</span>'}).join('');var isGoogle=Boolean(v.google_place);var href=isGoogle?(v.maps_url||'#'):('/app/vendor/'+encodeURIComponent(v.id));var target=isGoogle?' target="_blank" rel="noopener"':'';var price=isGoogle?'Google listing':('From $'+Number(v.price_from||0).toFixed(0));var ratingLabel=Number(v.avg_rating||0)?Number(v.avg_rating||0).toFixed(1):'—';return '<a class="vendor-card glass" data-vendor-id="'+encodeURIComponent(v.id)+'" href="'+escHtml(href)+'"'+target+'>'+(v.photo_url?'<img src="'+encodeURI(v.photo_url)+'" alt="'+escHtml(v.name)+'" loading="lazy">':'')+'<div class="vendor-card-body"><div class="vendor-card-meta"><span class="eyebrow">'+(isGoogle?'Google Maps':escHtml(String(v.category||'').replace(/_/g,' ')))+'</span><strong>'+price+'</strong></div><h3>'+escHtml(v.name)+'</h3><p class="vendor-summary">'+escHtml(v.summary)+'</p><div class="vendor-rating"><span class="stars">★★★★★</span><span>'+ratingLabel+' · '+Number(v.review_count||0)+' reviews</span></div><p class="muted">'+escHtml(v.location_address||'Jamaica')+'</p><div class="match-reasons">'+reasons+'</div></div></a>'}).join('');
