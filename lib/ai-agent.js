...
async function conciergeReply(message, history = [], opts = {}) {
  ...
  const style = spoken
    ? 'Speak like a warm, trusted Jamaican local guide (Tanty energy): answer the traveler question directly in 2-4 short sentences. Use natural Jamaican English with light Patois flavor (yeah mon, irie, walk good) — clear enough for visitors. Do NOT only restate or translate their question. Give a real recommendation or practical answer.'
    : 'You are PatWaGo AI, a warm Jamaica travel concierge. Answer the traveler question directly with practical local guidance.';
  const messages = [
    {
      role: 'system',
