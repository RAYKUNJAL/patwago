...
async function generateItinerary(profile = {}) {
  ...
  const messages = [
    {
      role: 'system',
      content: 'You are PatWaGo AI, a Jamaica travel itinerary specialist. Build practical plans only from the supplied PatWaGo catalog. Never invent attractions, vendors, opening hours, safety guarantees, or travel times. Use only listed place_id and vendor_ids. Adapt to mobility, budget, group, interests, location, and pace. Keep each day geographically sensible. If information is uncertain, say to confirm it.',
    },
    { role: 'user', content: JSON.stringify({ traveler: profile, places: compactPlaces, vendors: compactVendors }) },
  ];
  try {
...
async function conciergeReply(message, history = []) {
  ...
  const messages = [
    {
      role: 'system',
      content: `You are PatWaGo AI, a warm Jamaica travel concierge. Ground every factual recommendation in this catalog and identify uncertainty. Do not claim to book, guarantee safety, or know live hours. Keep spoken answers concise and useful. Catalog: ${JSON.stringify({ places, vendors: vendors.map((v) => ({ id: v.id, name: v.name, category: v.category, location: v.location_address, place_id: v.featured_place_id })) })}`,
    },
