...
const PLAN_DURATIONS_MS = {
  ...
  day: 24 * 3600000, // ~24h
  week: 7 * 24 * 3600000, // fixed 7 days
...
async function createPass({ customer_id, plan, trip_days } = {}) {
  if (!isNonEmptyString(customer_id)) throw new Error('customer_id is required');
  if (!VALID_PLANS.has(plan)) throw new Error('plan must be one of: trial, day, week, trip');

  // Verify the customer exists (fail closed).
  const customer = await getCustomerById(customer_id);
  if (!customer) throw new Error('customer not found');

  // One free trial per account — paid passes can renew.
  if (plan === 'trial') {
    const existing = await listPasses(customer_id);
    if (existing.some((p) => p.plan === 'trial')) {
      throw new Error('Free trial already used on this account. Choose a Day or Trip pass to continue.');
    }
  }
...
async function getAccessSummary(customer_id) {
  const passes = await listPasses(customer_id);
  const now = nowMs();
  const live = passes.filter((p) => new Date(p.expires_at).getTime() >= now);
  const best = live.sort((a, b) => new Date(b.expires_at) - new Date(a.expires_at))[0] || null;
  return {
    active: Boolean(best),
    had_trial: passes.some((p) => p.plan === 'trial'),
    paid_active: live.some((p) => p.plan !== 'trial'),
    is_trial: Boolean(best && best.plan === 'trial'),
    pass: best,
    passes,
  };
}
